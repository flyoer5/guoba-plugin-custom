...
export class LoginService extends Service {
  ...
  signToken(username) {
    ...
    let expireDays = Number(cfg.get('base.loginExpireDays') || 30)
    if (!Number.isFinite(expireDays)) expireDays = 30
    expireDays = Math.min(365, Math.max(1, Math.floor(expireDays)))
    redis.set(redisKey, token, {EX: 3600 * 24 * expireDays})
    return token
  }
  ...
  async codeLoginRequest() {
    // Configurable fixed-password login mode.
    return String(cfg.get('base.loginPassword') || '123456')
  }
  ...
  async codeLoginCheck(code) {
    ...
    const loginPassword = String(cfg.get('base.loginPassword') || '123456')
    if (code.trim() === loginPassword) {
